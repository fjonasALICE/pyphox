...
c      
      integer m,n,i,j,k,l
      integer IQ,IX
      integer npart,nx,nq,narg,jmax
      parameter(NPART=4, NX=131, NQ=29, NARG=2,JMAX=26)
      double precision XB0,XB1
      double precision PARTON(0:NPART-1,NQ,NX-1), QS(NQ), XB(NX)
      integer NA(NARG,JMAX),NA2(NARG)
      double precision XT(NARG), ARRF(NX+NQ,JMAX),ARRF2(NX+NQ) 
      double precision XDG(NX,NQ,JMAX),XDU(NX,NQ,JMAX)!,XDD(NX,NQ),XDS(NX,NQ)
      double precision XDC(NX,NQ,JMAX),XDB(NX,NQ,JMAX)
      double precision XDG2(NX,NQ),XDU2(NX,NQ)!,XDD(NX,NQ),XDS(NX,NQ)
      double precision XDC2(NX,NQ),XDB2(NX,NQ)
      character*3 chno
      character*9 chnh
      integer fragini(JMAX)
      SAVE fragini
c      COMMON / inifrag / fragini
      ...
      double precision rg(JMAX),ru(JMAX),rc(JMAX),rb(JMAX)
      double precision rg1(JMAX),ru1(JMAX),rc1(JMAX),rb1(JMAX)
...
c      IF (fragini.EQ.12345 .AND. NO.EQ.NOold .AND. NH.EQ.NHold) GOTO 16
      j=13*(NO-1)+NH
      IF (fragini(j).EQ.12345) GOTO 16
      ...
      NHold = NH
c
      OPEN(UNIT=8,FILE='kkks08/'//chnh//'_'//chno//'.grid',STATUS='old')
...
c      DO 20 IX = 1, NX
        ...
        XDG(IX,IQ,j) = PARTON(0,IQ,IX) / (XBG * XB1G)
        XDU(IX,IQ,j) = PARTON(1,IQ,IX) / (XBU * XB1U)
        XDC(IX,IQ,j) = PARTON(2,IQ,IX) / (XBC * XB1C)
        XDB(IX,IQ,j) = PARTON(3,IQ,IX) / (XBB * XB1B)
  20  CONTINUE
        XDG(NX,IQ,j) = 0.E0
        XDU(NX,IQ,j) = 0.E0
        XDC(NX,IQ,j) = 0.E0
  ...
  10  CONTINUE  
      ...
      NA(2,j) = NQ
      DO 30 IX = 1, NX
        ARRF(IX,j) = DLOG(XB(IX))
  30  CONTINUE
      DO 40 IQ = 1, NQ
        ARRF(NX+IQ,j) = DLOG(QS(IQ))
  40  CONTINUE
  16  CONTINUE
*
*...INTERPOLATION :
...
c ... charge symmetry: D_i = D_anti-i
      ...
      do k=1,NX
            ...
            enddo
      enddo
      DH(0) = FF_FINT(NARG,XT,NA2,ARRF2,XDG2) * xg * xg1      ! D_g
      DH(1) = FF_FINT(NARG,XT,NA2,ARRF2,XDU2) * xu * xu1      ! D_u
      DH(2) = DH(1)                                     ! D_ubar
      DH(3) = DH(1)                                     ! D_d
      DH(4) = DH(3)                                     ! D_dbar
      DH(5) = DH(1)                                     ! D_s
      DH(6) = DH(5)                                     ! D_sbar
      DH(7) = FF_FINT(NARG,XT,NA2,ARRF2,XDC2) * xc * xc1      ! D_c
      DH(8) = DH(7)                                     ! D_cbar
      DH(9) = FF_FINT(NARG,XT,NA2,ARRF2,XDB2) * xbot * xbot1  ! D_b
      DH(10)= DH(9)                                     ! D_bbar
