...
C READ GRID DATA AND CALCULATE SPLINE COEFFICIENTS. 
        ...
        DO I=1,12
          DO J=1,12
            EM(I,J)=EMI(I,J,IORDER)
          ENDDO
        ENDDO

      IF(IORDER.EQ.1)THEN 
        OPEN(UNIT=IFILE,FILE='hkn07/'//FNPDF_LO(ISET),
     *STATUS='OLD')
        OPEN(UNIT=IFILE+1,FILE='hkn07/'//FGRAD_LO(ISET),
     *STATUS='OLD')
      ELSE IF(IORDER.EQ.2)THEN
        OPEN(UNIT=IFILE,FILE='hkn07/'//FNPDF_NLO(ISET),
     *STATUS='OLD')
        OPEN(UNIT=IFILE+1,FILE='hkn07/'//FGRAD_NLO(ISET),
     *STATUS='OLD')
      END IF

      DO J=1,NQ
        DO K=1,NX-1
          READ(IFILE,1025) (PDFG(K,J,I), I=1,NFF)
          DO NR=1,NPAR
            NI=10+NFF*(NR-1)
 ...
 1025 FORMAT(1X,7(1PE14.6))
      ...
      DO I=1,ND
        DO J=1,NQ
          PDFG(NX,J,I)=0.D0 ! x=1 NPDF=0.D0
...
C -- Gradient terms of each parameters for Nuclear PDF --
      ...
      END
C ---------------------------------------------------------------------
C       SUBROUTINE LSPLINE(N,X,Y,B,C,D,ISET,I,J)
C C ---------------------------------------------------------------------
