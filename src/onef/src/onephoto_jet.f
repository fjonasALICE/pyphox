...
C modif le 28/10/02 pour prendre en compte les effets du ptmin du jet
	subroutine onef_sub
       use luxury
...
c coupure en isolement-------------------------------------------------
	...
	fi45 = zacos(cos_fi45)
c	call isolement(y4,y5,fi45,x4,pt5,gpt4,r_isol,etmax,
c     #	isol)
	call isolement(y4,y5,fi45,x4,pt5,gpt4,isol)
	xisol = 1.d0
	if (.not.(isol)) then
	  if (icorr.eq.0) then
	    f4dimo3 = 0.d0
...
c
	...
	if (icorr.eq.1) then
c coupure en isolement-------------------------------------------------
c	  call isolement(y4,y3,pi,x4,pt5,gpt4,r_isol,etmax,
c     #	  isol)
	  call isolement(y4,y3,pi,x4,pt5,gpt4,isol)
	  xisolp = 1.d0
	  if (.not.(isol)) then
	    xisolp = 0.d0
	  endif
...
c
	...
	if (icorr.eq.1) then
c coupure en isolement-------------------------------------------------
c	  call isolement(y4,y4,0.d0,x4,pt5,gpt4,r_isol,etmax,
c     #	  isol)
	  call isolement(y4,y4,0.d0,x4,pt5,gpt4,isol)
	  xisolp = 1.d0
	  if (.not.(isol)) then
	    xisolp = 0.d0
	  endif
...
c coupure en isolement-------------------------------------------------
c	call isolement(0.d0,0.d0,0.d0,x4,0.d0,gpt4,r_isol,etmax,
c     #	isol)
	call isolement(0.d0,0.d0,0.d0,x4,0.d0,gpt4,isol)
	if (.not.(isol)) then
	  f2dimo1 = 0.d0
	  return
	endif
...
c ------------------ gpt3 -------------------------------------------
	if (i_flag_iso.eq.1) then
	  gpt3maxp = dmin1(gpt_jetsup,dsqrt(s)/(2.d0*dcosh(y3))
     #	  ,dsqrt(s)/(2.d0*dcosh(y4)),gpt4+etmax(1))
	else if (i_flag_iso.eq.2) then
	  gpt3maxp = dmin1(gpt_jetsup,dsqrt(s)/(2.d0*dcosh(y3))
     #	  ,dsqrt(s)/(2.d0*dcosh(y4)),gpt4*(1.d0+etmax(1)))
	else if (i_flag_iso.eq.3) then
	  gpt3maxp = dmin1(gpt_jetsup,dsqrt(s)/(2.d0*dcosh(y3))
     #	  ,dsqrt(s)/(2.d0*dcosh(y4)),gpt4+etmax(ieff_nb_cone))
	else if (i_flag_iso.eq.4) then
	  gpt3maxp = dmin1(gpt_jetsup,dsqrt(s)/(2.d0*dcosh(y3))
     #	  ,dsqrt(s)/(2.d0*dcosh(y4)),gpt4*(1.d0+etmax(ieff_nb_cone)))
	else if (i_flag_iso.eq.5) then
	  gpt3maxp = dmin1(gpt_jetsup,dsqrt(s)/(2.d0*dcosh(y3))
     #	  ,dsqrt(s)/(2.d0*dcosh(y4)),gpt4*(1.d0+etmax(1))+etmax(2))
	endif
	gpt3max = dmax1(gpt_jetinf,gpt4,gpt3maxp)
	gpt3min = dmax1(gpt_jetinf,gpt4)
	gpt3 = gpt3min + (gpt3max-gpt3min)*xx(4)	
c ------------------ x4 -------------------------------------------
	x4max = 1.d0
...
c juste pour tester (en principe inutile)
	...
	x4 = gpt4/(gpt3*z4)
c juste pour tester (en principe inutile)
	if (x4.gt.x4max.or.x4.lt.x4minpp) then
	  f2dimo3 = 0.d0
	  write (18,*) 'attention x4 > x4min ou x4 < x4max',
     #	              x4,x4minpp,x4max
	  return
	endif
	pt4 = gpt4/x4
...
c	call isolement(0.d0,0.d0,0.d0,x4,pt5,gpt4,r_isol,etmax,
c     #	isol)
	call isolement(0.d0,0.d0,0.d0,x4,pt5,gpt4,isol)
	if (.not.(isol)) then
	  f2dimo3 = 0.d0
	  return
	endif
...
c c^b_{ij} = 2*pi*alpha_s*alpha/(4*c_i*c_j*s**2)
	...
	alpi = alphas/(2.d0*pi)
c
	call ssy4po(s,gpt3,gpt4,y3,y4,mf,x10,x20,z4,ih1,ih2,ih4,sy4p)
	if (z4.le.z4sup) then
	  call ssy4ro(s,gpt3,gpt4,y3,y4,r,x10,x20,z4,ih1,ih2,ih4,sy4r)
	else
...
c 
	if (iborn.eq.0) then 
     ...
     #	  c0sv12)
	  call ssv4o(s,gpt4,gpt_jetsup,y3,y4,mf,x10,x20,x4,
     #	  ih1,ih2,ih4,c0sv4)
